...
import Infinitygroup.imersive_cam.api.client.IImersiveCamCamera;
...
public class DebugScreenOverlayHandler {
	public static void appendDebugText(List<String> lines) {
		IImersiveCam instance = ImersiveCam.getInstance();
		if (instance.isImersiveCam() && !Minecraft.getInstance().showOnlyReducedInfo() && instance.isCameraDecoupled()) {
			int index = findFacingDebugTextIndex(lines);
			if (index != -1) {
				IImersiveCamCamera camera = instance.getCamera();

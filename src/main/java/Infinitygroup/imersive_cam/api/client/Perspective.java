...
import Infinitygroup.imersive_cam.api.config.IPerspectiveConfig;
...
public enum Perspective {
	...
	IMERSIVE_CAMERA(CameraType.THIRD_PERSON_BACK, CrosshairVisibility.ALWAYS);
	...
	public Perspective next(IPerspectiveConfig config) {
		...
		if (config.isThirdPersonReplaced()) {
			next = switch (this) {
				case FIRST_PERSON, THIRD_PERSON_BACK -> IMERSIVE_CAMERA;
				case THIRD_PERSON_FRONT -> FIRST_PERSON;
				case IMERSIVE_CAMERA -> THIRD_PERSON_FRONT;
			};
		} else {
		...
		switch (next) {
			...
			case THIRD_PERSON_FRONT:
				...
				break;
			case IMERSIVE_CAMERA:
				return IMERSIVE_CAMERA;
		}
	...
	public boolean isEnabled(IPerspectiveConfig config) {
		return switch (this) {
			...
			case IMERSIVE_CAMERA -> true;
		};
	}
	
	public static Perspective of(CameraType cameraType, boolean immersiveCameraEnabled) {
		return switch (cameraType) {
			case FIRST_PERSON -> Perspective.FIRST_PERSON;
			case THIRD_PERSON_BACK -> immersiveCameraEnabled ? Perspective.IMERSIVE_CAMERA : Perspective.THIRD_PERSON_BACK;
			case THIRD_PERSON_FRONT -> Perspective.THIRD_PERSON_FRONT;
		};
	}
	
	public static Perspective current() {
		return Perspective.of(Minecraft.getInstance().options.getCameraType(), IImersiveCam.getInstance().isImersiveCam());
	}
}

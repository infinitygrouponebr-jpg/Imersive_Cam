...
public class CameraConfig implements ICameraConfig {
	// Tactical shoulder-camera baseline:
	// offsetX controls lateral placement, offsetY controls height and offsetZ controls distance.
	// These defaults aim for a HumanitZ-like feel, but slightly closer and steadier for Minecraft.
	private static final double DEFAULT_TACTICAL_OFFSET_X = -0.95D;
	private static final double DEFAULT_TACTICAL_OFFSET_Y = 1.15D;
	private static final double DEFAULT_TACTICAL_OFFSET_Z = 3.35D;
	// Higher transition speed means less lag and a more stable over-the-shoulder camera.
	...
	protected CameraConfig(ModConfigSpec.Builder builder) {
		...
		this.offsetX = builder
			.comment("Third person camera x-offset. Negative values place the camera over the right shoulder; positive values move it left.")
		...
		this.offsetY = builder
			.comment("Third person camera y-offset. Higher values lift the camera above the shoulder and closer to head level.")
		...
		this.offsetXPresets = builder
			.comment("A list of x-offset presets that can be toggled via the 'Toggle X-Offset Presets' keybind. These defaults stay near the right shoulder for a tactical third-person view. WARNING: Duplicate entries can result in undefined behavior!")
			.translation(MOD_ID + ".configuration.camera.offset.presets.offset_x_presets")
			.defineList("presets_offset_x", () -> new ArrayList<String>(List.of("-0.80", "-0.95", "-1.10")), String::new, ClientConfig::isValidDouble);
		
		this.offsetYPresets = builder
			.comment("A list of y-offset presets that can be toggled via the 'Toggle Y-Offset Presets' keybind. These defaults keep the camera slightly above the shoulder without pushing into a top-down angle. WARNING: Duplicate entries can result in undefined behavior!")
			.translation(MOD_ID + ".configuration.camera.offset.presets.offset_y_presets")
			.defineList("presets_offset_y", () -> new ArrayList<String>(List.of("0.95", "1.15", "1.35")), String::new, ClientConfig::isValidDouble);
		
		this.offsetZPresets = builder
			.comment("A list of z-offset presets that can be toggled via the 'Toggle Z-Offset Presets' keybind. These defaults stay in the medium-short tactical range for tighter indoor visibility. WARNING: Duplicate entries can result in undefined behavior!")
			.translation(MOD_ID + ".configuration.camera.offset.presets.offset_z_presets")
			.defineList("presets_offset_z", () -> new ArrayList<String>(List.of("3.00", "3.35", "3.70")), String::new, ClientConfig::isValidDouble);
